...
def test_delete_certificate_requires_authentication(session):
...
def test_generate_csr_works(session):
...
def test_generate_self_signed_certificate_requires_pin(session):
    session.verify_pin(DEFAULT_PIN)
    public_key = generate_key(session, SLOT.AUTHENTICATION)
...
def _test_generate_self_signed_certificate(session, slot):
...
def test_generate_self_signed_certificate_slot_9a_works(session):
    _test_generate_self_signed_certificate(session, SLOT.AUTHENTICATION)


def test_generate_self_signed_certificate_slot_9c_works(session):
    _test_generate_self_signed_certificate(session, SLOT.SIGNATURE)


def test_generate_key_requires_authentication(session):
    with pytest.raises(ApduError):
...
def test_put_certificate_requires_authentication(session):
    cert = get_test_cert()
    with pytest.raises(ApduError):
...
def _test_put_key_pairing(session, alg1, alg2):
...
@condition(not_roca)
def test_put_certificate_verifies_key_pairing_rsa1024(session):
    _test_put_key_pairing(session, KEY_TYPE.RSA1024, KEY_TYPE.ECCP256)


@condition(not_roca)
def test_put_certificate_verifies_key_pairing_rsa2048(session):
    _test_put_key_pairing(session, KEY_TYPE.RSA2048, KEY_TYPE.ECCP256)


@condition(not_roca)
def test_put_certificate_verifies_key_pairing_eccp256_a(session):
    _test_put_key_pairing(session, KEY_TYPE.ECCP256, KEY_TYPE.RSA2048)


@condition.min_version(4)
def test_put_certificate_verifies_key_pairing_eccp256_b(session):
    _test_put_key_pairing(session, KEY_TYPE.ECCP256, KEY_TYPE.ECCP384)


@condition.min_version(4)
def test_put_certificate_verifies_key_pairing_eccp384(session):
    _test_put_key_pairing(session, KEY_TYPE.ECCP384, KEY_TYPE.ECCP256)


def test_put_key_requires_authentication(session):
...
def test_get_certificate_does_not_require_authentication(session):
    cert = get_test_cert()
    session.authenticate(MANAGEMENT_KEY_TYPE.TDES, DEFAULT_MANAGEMENT_KEY)
    session.put_certificate(SLOT.AUTHENTICATION, cert)
    reset_state(session)
...
def test_authenticate_twice_does_not_throw(session):
    session.authenticate(MANAGEMENT_KEY_TYPE.TDES, DEFAULT_MANAGEMENT_KEY)
    session.authenticate(MANAGEMENT_KEY_TYPE.TDES, DEFAULT_MANAGEMENT_KEY)


def test_reset_resets_has_stored_key_flag(session):
...
def disabled_test_reset_while_verified_throws_nice_ValueError(session):
    session.verify_pin(DEFAULT_PIN)
...
def test_set_mgm_key_does_not_change_key_if_not_authenticated(session):
    with pytest.raises(ApduError):
...
def test_set_stored_mgm_key_does_not_destroy_key_if_pin_not_verified(session):
    session.authenticate(MANAGEMENT_KEY_TYPE.TDES, DEFAULT_MANAGEMENT_KEY)
    with pytest.raises(ApduError):
        pivman_set_mgm_key(
            session,
...
def test_set_mgm_key_changes_mgm_key(session):
    session.authenticate(MANAGEMENT_KEY_TYPE.TDES, DEFAULT_MANAGEMENT_KEY)
    session.set_management_key(MANAGEMENT_KEY_TYPE.TDES, NON_DEFAULT_MANAGEMENT_KEY)
...
def test_set_stored_mgm_key_succeeds_if_pin_is_verified(session):
    session.verify_pin(DEFAULT_PIN)
    session.authenticate(MANAGEMENT_KEY_TYPE.TDES, DEFAULT_MANAGEMENT_KEY)
    pivman_set_mgm_key(
        session,
...
@condition.min_version(4)
def test_sign_with_pin_policy_always_requires_pin_every_time(session):
...
@condition.min_version(4)
def test_sign_with_pin_policy_never_does_not_require_pin(session):
...
@condition.fips()
def test_pin_policy_never_blocked_on_fips(session):
    with pytest.raises(ApduError):
        generate_key(session, pin_policy=PIN_POLICY.NEVER)


@condition.min_version(4)
def test_sign_with_pin_policy_once_requires_pin_once_per_session(session):
...
def test_signature_can_be_verified_by_public_key(session):
...
def test_unblock_pin_requires_no_previous_authentication(session):
    session.unblock_pin(DEFAULT_PUK, NON_DEFAULT_PIN)


def test_unblock_pin_with_wrong_puk_throws_InvalidPinError(session):
    with pytest.raises(InvalidPinError):
        session.unblock_pin(NON_DEFAULT_PUK, NON_DEFAULT_PIN)


def test_unblock_pin_resets_pin_and_retries(session):
    session.reset()
    reset_state(session)
...
def test_set_pin_retries_requires_pin_and_mgm_key(session):
...
def test_set_pin_retries_sets_pin_and_puk_tries(session):
    pin_tries = 9
    puk_tries = 7
