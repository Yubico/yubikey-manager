...
use crate::py_bridge::{scp_key_params_from_py, smartcard_err, PySmartCardConnection};
...
impl PyYubiOtpSession {
    ...
    fn new(connection: &Bound<'_, PyAny>, scp_key_params: Option<&Bound<'_, PyAny>>) -> PyResult<Self> {
        ...
        if let Some(params) = scp_key_params {
            ...
            let session = RustYubiOtpSession::new_with_scp(conn, &scp_params).map_err(yubiotp_err)?;
            Ok(Self { session })
        } else {
