...
You can write custom Python scripts using the yubikey-manager library to
interact with YubiKeys.
...
=== Invoking the script
To run a script, use the Python interpreter directly (with `yubikit` and `ykman`
installed):
...
of the command:
  ...
  python myscript.py 123456 a_word "a string with spaces"
...
Save the file, then run it again, and you should see output similar to:
...
try:
    ...
    print("Usage: python myscript.py <serial>")
    sys.exit(1)
...
....
> python myscript.py
Usage: python myscript.py <serial>
>
....
