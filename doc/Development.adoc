...
You will also need a Rust toolchain (stable). Install via https://rustup.rs/[rustup].
...
No additional system packages required (PC/SC is provided by the OS).
...
This project uses https://docs.astral.sh/ruff/[ruff] for Python code style and
security linting. https://mypy-lang.org[mypy] and
https://github.com/microsoft/pyright[pyright] are used for static type checking.
