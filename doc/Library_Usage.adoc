== Using yubikit as a Python library
...
=== Setup
You will need to have Python >= 3.10, and have `yubikit` installed and added
to your PYTHON_PATH. You can verify that this is set up correctly by running
the following command from a Terminal:

    $ python -c "import yubikit"
...
YubiKey, reading some information about it. It returns a list of YubiKeyDevice
objects. Each device has an `info` property which tells you what kind of
YubiKey it is, what capabilities it has, its serial number, etc. The device
also lets you open a Connection to it, which lets you interact with one of the
...
for device in list_all_devices():
    if device.info.version >= (5, 0, 0):
        print(f"Found YubiKey with serial number: {device.info.serial}")
----
...
while True:  # Run this until we stop the script with Ctrl+C
    ...
    if new_state != state:
        ...
        for device in list_all_devices():
            if device.info.serial not in handled_serials:  # Unhandled YubiKey
                print(f"Programming YubiKey with serial: {device.info.serial}")
