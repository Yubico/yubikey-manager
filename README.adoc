...
You may run into MacOS permission problems resulting in an error such as:

  Failed to open device for communication: -536870174
  
Try to add your terminal application to the `Input Monitoring` permission list via `System Preferences -> Security & Privacy -> Privacy -> Input Monitoring` to resolve this error.
