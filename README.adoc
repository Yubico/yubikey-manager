...
Usage: ykman [OPTIONS] [COMMAND]

Commands:
  list     List connected YubiKeys
  info     Show general information
  config   Enable or disable applications and settings
  oath     Manage the OATH application
  otp      Manage the YubiKey OTP application
  piv      Manage the PIV application
  fido     Manage the FIDO applications
  openpgp  Manage the OpenPGP application
  hsmauth  Manage the YubiHSM Auth application
  sd       Manage the Security Domain
  apdu     Send raw APDUs to the YubiKey
  help     Print this message or the help of the given subcommand(s)

Options:
  -d, --device <DEVICE>              Specify which YubiKey to interact with by serial number
      --scp <SCP_CRED>               SCP credentials: private key and cert files, or SCP03 keys as K-ENC:K-MAC[:K-DEK] hex
      --scp-ca <SCP_CA>              CA certificate for SCP11 card key verification (PEM/DER file)
      --scp-sd <KID> <KVN>           Card key reference for SCP (KID KVN, hex)
      --scp-oce <KID> <KVN>          OCE key reference for SCP (KID KVN, hex)
      --scp-password <SCP_PASSWORD>  Password for SCP credential file
      --diagnose                     Show diagnostic information
  -l, --log-level <LOG_LEVEL>        Enable logging at given verbosity level [possible values: error, warning, info, debug, traffic]
...
YubiKey Manager for Python 3.10, use:

  # pkg install py310-yubikey-manager
